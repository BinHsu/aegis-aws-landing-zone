...
| [008](008-workload-multi-region.md) | Workload multi-region DR (active-passive pilot light) | AWS region outage | ~$2–4 / session (Mode A, depending on workloads doubling) or ~$1 / month (Mode B persistent) | Partially implemented — schema + modules (network + platform + workloads, all slot-patterned) + CI plan matrix + K=2 guard done (Session A+B, 2026-04-19/20); Session C apply/teardown pending |
