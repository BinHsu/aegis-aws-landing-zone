# -----------------------------------------------------------------------------
# Detective Controls — ADR-016 Item A (SHIPPED)
# -----------------------------------------------------------------------------
...
# complement to ADR-014 (CI OIDC role scope-down) and ADR-015 (permission-
# boundary hardening), both preventive.
# Mgmt-account scope only at MVP; staging + shared deferred per ADR-016 Item B.
# -----------------------------------------------------------------------------
...
resource "aws_sns_topic" "security_alerts" {
  # SNS topic for the detective alerts. Subscribers receive a notification
  # whenever the failed-OIDC-assumption rule (or any future Item B/C rule)
  # matches an event. KMS-encrypted with the AWS-managed SNS key per ADR-016
...
resource "aws_sns_topic_subscription" "security_alerts_email" {
  ...
  # mailbox per ADR-016 OQ-2 (one less config field for forkers at MVP).
...
resource "aws_cloudwatch_event_rule" "failed_oidc_assumption" {
  ...
  description = "Detective control: alert on failed AssumeRoleWithWebIdentity events (potential OIDC trust policy bypass attempt). ADR-016."
