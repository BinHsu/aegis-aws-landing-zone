...
resource "aws_iam_role_policy" "gh_tf_apply_baseline" {
  ...
  # checkov:skip=CKV_AWS_355: Resource:* is by design on the read-only Sid and on AWS APIs that do not support resource-level ARNs (Organizations, SSO, RAM, tag, IPAM-create-SLR). Every mutating action with Resource:* is service-namespace-scoped at the action prefix and gated by the trust policy `sub: ref:refs/heads/main`.
