...
After the workloads slot-pattern refactor (PR #(pending), 2026-04-20), each cluster has its own independent Grafana with its own `random_password`. Outputs are flat per slot — `grafana_admin_password_primary` (always present) and `grafana_admin_password_slave_1` (null when `eks.staging.regions` is length 1).
