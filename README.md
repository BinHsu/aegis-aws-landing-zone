...
The Grafana admin password is generated by Terraform (`random_password`) and surfaced as a sensitive output on the `staging/workloads` layer. The password lives in Terraform state (S3 + KMS in the shared account); the state backend is the trust boundary.

After the workloads slot-pattern refactor ([PR #100](https://github.com/BinHsu/aegis-aws-landing-zone/pull/100), 2026-04-20), each cluster has its own independent Grafana with its own `random_password`. Outputs are flat per slot — `grafana_admin_password_primary` (always present) and `grafana_admin_password_slave_1` (null when `eks.staging.regions` is length 1).

**Current role**: admin is both break-glass *and* daily auth, because SSO/OIDC is not yet wired to Grafana. See [`docs/improvements/009-grafana-sso-integration.md`](docs/improvements/009-grafana-sso-integration.md) for the roadmap; once that ships, `grafana.admin.disableLogin = true` makes this output true break-glass.

**Retrieve (no shell-history leak)**:

```bash
aws sso login --profile aegis-staging-admin
cd terraform/environments/staging/workloads
AWS_PROFILE=aegis-staging-admin terraform init -input=false   # first time only

# Primary cluster Grafana
AWS_PROFILE=aegis-staging-admin terraform output -raw grafana_admin_password_primary | pbcopy

# Slave_1 cluster Grafana (only when eks.staging.regions length is 2)
AWS_PROFILE=aegis-staging-admin terraform output -raw grafana_admin_password_slave_1 | pbcopy
```

**Regenerates** on every fresh `terraform apply` of the workloads layer. Intentional — the password is ephemeral; don't treat it as stable across sessions.

**No out-of-band recovery**: if you lose state access you lose the password. This is the reverse of "you can always read the state"; SSO for humans is the only scalable answer, which is why #009 exists.
